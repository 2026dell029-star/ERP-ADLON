...
export const ReportCardModal: React.FC<ReportCardModalProps> = ({
  ...
  const handlePrint = () => {
    setPrintNotice('Lancement de l\'impression directe...');
    setTimeout(() => setPrintNotice(null), 3500);

    try {
      window.print();
    } catch (err) {
      console.warn('Native window.print failed, opening in printable tab', err);
      openPrintableReportInNewTab(report, student, config, true);
    }
  };

  const handleOpenNewTab = () => {
    setPrintNotice('Ouverture du document officiel dans un nouvel onglet...');
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-slate-950/70 backdrop-blur-xs overflow-y-auto print:p-0 print:bg-white print:static print:overflow-visible">
      <div className="relative w-full max-w-4xl bg-white dark:bg-[#111827] rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 flex flex-col my-auto max-h-[94vh] print:max-h-none print:border-none print:shadow-none print:w-full print:rounded-none">
        
        {/* Modal Action Header (Hidden during Print) */}
        <div className="print:hidden flex flex-wrap items-center justify-between gap-3 px-4 sm:px-6 py-3.5 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-[#0B0F17] rounded-t-2xl">
          {/* Left: Term Switcher */}
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={handlePrint}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-900 dark:bg-white text-white dark:text-slate-950 hover:bg-slate-800 dark:hover:bg-slate-100 text-xs font-bold shadow-xs transition-colors cursor-pointer"
              title="Lancer l'impression directe (A4 Portrait)"
            >
              <Printer className="w-3.5 h-3.5 text-blue-400 dark:text-blue-600" />
              <span>Imprimer</span>
            </button>
        ...
        <div className="overflow-y-auto overscroll-contain flex-1 p-4 sm:p-6 md:p-8 bg-slate-100/50 dark:bg-[#0d121c] print:p-0 print:bg-white print:overflow-visible">
          <div 
          ...
          >
            ...
            <div className="flex h-1.5 w-full mb-4 rounded-full overflow-hidden print:h-1">
            ...
            <div className="border-b-2 border-slate-900 dark:border-slate-600 pb-4 print:border-black">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3 items-center text-center md:text-left print:grid-cols-3">
                ...
                <div className="text-[10px] leading-snug text-slate-600 dark:text-slate-300 print:text-black">
                  <p className="font-extrabold uppercase tracking-wider text-slate-900 dark:text-white print:text-black text-xs">
                    {schoolCountry}
                  </p>
                ...
                <div className="text-center">
                  {config?.schoolLogo ? (
                    <div className="inline-flex items-center justify-center w-12 h-12 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-1 mb-1 shadow-xs overflow-hidden print:border-black print:bg-white">
                      <img
                  ...
                  <p className="text-[9.5px] text-slate-500 dark:text-slate-400 print:text-black">
                ...
                <div className="text-center md:text-right print:text-right text-xs">
                  <div className="inline-block p-2 rounded-xl bg-slate-100 dark:bg-slate-900/80 border border-slate-300 dark:border-slate-700 print:border-black print:bg-white text-left text-[10.5px]">
                    <p className="font-bold text-[9.5px] text-slate-500 dark:text-slate-400 print:text-black">ANNÉE SCOLAIRE :</p>
                    <p className="font-mono font-bold text-xs text-slate-900 dark:text-white print:text-black">{academicYear}</p>
                    <p className="font-bold text-[9.5px] text-slate-500 dark:text-slate-400 print:text-black mt-1">PÉRIODE :</p>
              ...
              <div className="mt-3.5 py-1.5 px-4 rounded-lg bg-slate-900 dark:bg-white text-white dark:text-slate-900 text-center font-black tracking-widest text-xs uppercase shadow-xs print:bg-slate-100 print:text-black print:border print:border-black">
                BULLETIN OFFICIEL DE NOTES & BILAN PÉDAGOGIQUE
              </div>
            </div>

            {/* Student Administrative Identification Card */}
            <div className="mt-4 p-4 rounded-xl bg-slate-50 dark:bg-slate-900/60 border border-slate-300 dark:border-slate-700 print:border-black print:bg-white">
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
                <div>
                  <span className="text-[9.5px] font-bold text-slate-500 dark:text-slate-400 print:text-black block uppercase">
                    Nom & Prénoms :
                  </span>
              ...
              <div className="mt-3 pt-2.5 border-t border-slate-200 dark:border-slate-800 print:border-black grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs items-center">
                <div>
                ...
                <div>
                  <span className="text-[10px] text-slate-500 dark:text-slate-400 print:text-black">Note de conduite :</span>
                  <span className="font-mono font-bold ml-1 text-slate-900 dark:text-white print:text-black">{report.conductScore} / 20</span>
                </div>

                <div>
                  <span className="text-[10px] text-slate-500 dark:text-slate-400 print:text-black">Absences non justifiées :</span>
                ...
                <div>
                  <span className="text-[10px] text-slate-500 dark:text-slate-400 print:text-black">Retards constatés :</span>
            ...
            {/* Grades Table */}
            <div className="mt-4 overflow-x-auto rounded-xl border border-slate-300 dark:border-slate-700 print:border-black">
              <table className="w-full text-left text-xs border-collapse">
                <thead>
                  <tr className="bg-slate-900 text-white dark:bg-slate-800 border-b border-slate-300 dark:border-slate-700 print:bg-slate-200 print:text-black print:border-black font-bold">
                    ...
                    <th className="py-2.5 px-2 text-center hidden sm:table-cell">Moy. Cl.</th>
                  ...
                  {report.grades.map((g, idx) => {
                    ...
                    return (
                      <tr key={g.subjectId || idx} className="hover:bg-slate-50 dark:hover:bg-slate-800/40">
                        <td className="py-2 px-3 font-bold text-slate-900 dark:text-slate-100 print:text-black">
                          {g.subjectName}
                        </td>
                        <td className="py-2 px-2 text-[11px] text-slate-500 dark:text-slate-400 print:text-black hidden sm:table-cell">
                        ...
                        <td className="py-2 px-2 text-center font-mono text-[11px] text-slate-500 dark:text-slate-400 print:text-black hidden sm:table-cell">
                        ...
                        <td className="py-2 px-3 text-[11px] text-slate-600 dark:text-slate-300 italic print:text-black">
                          {g.teacherRemark || (
                            g.score >= 16 ? 'Excellent travail, très bonne assimilation des acquis.' :
                            g.score >= 14 ? 'Très bon travail, participation active en classe.' :
                            g.score >= 12 ? 'Assez bien, les efforts réguliers portent leurs fruits.' :
                ...
                <tfoot>
                  <tr className="bg-slate-100 dark:bg-slate-900 font-bold border-t-2 border-slate-400 dark:border-slate-700 print:border-black print:bg-slate-100">
                    ...
                    <td className="py-2.5 px-2 text-center font-mono text-xs text-slate-900 dark:text-white print:text-black">
                      {report.totalCoefficients}
                    </td>
                    <td className="py-2.5 px-3 text-center font-mono text-sm font-extrabold text-[#0071E3] dark:text-[#38BDF8] print:text-black">
                      {report.totalPoints.toFixed(1)} pts
                    </td>
                    <td className="hidden sm:table-cell"></td>
                    <td className="text-[10px] text-slate-500 dark:text-slate-400 italic">
            ...
            <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-900/60 border border-slate-300 dark:border-slate-700 space-y-2.5 print:border-black print:bg-white">
                <h4 className="text-xs font-bold uppercase tracking-wider text-slate-600 dark:text-slate-300 print:text-black border-b border-slate-200 dark:border-slate-800 pb-1.5 flex justify-between">
                ...
                <div className="flex items-center justify-between text-xs">
                  ...
                  <span className="font-extrabold text-sm text-amber-700 dark:text-amber-400 print:text-black">
              ...
              <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-900/60 border border-slate-300 dark:border-slate-700 space-y-2.5 print:border-black print:bg-white">
                ...
                <div className="grid grid-cols-3 gap-2 text-center">
                  <div className="p-1.5 rounded-lg bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 print:border-black">
                    <span className="text-[9.5px] text-slate-500 dark:text-slate-400 print:text-black block">Moyenne</span>
                  ...
                  <div className="p-1.5 rounded-lg bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800/40 print:border-black">
                    <span className="text-[9.5px] text-emerald-700 dark:text-emerald-400 print:text-black block">Plus Forte</span>
                  ...
                  <div className="p-1.5 rounded-lg bg-rose-50 dark:bg-rose-950/30 border border-rose-200 dark:border-rose-800/40 print:border-black">
                    <span className="text-[9.5px] text-rose-700 dark:text-rose-400 print:text-black block">Plus Faible</span>
                ...
                <div className="text-[10.5px] text-slate-600 dark:text-slate-300 print:text-black pt-1">
            ...
            <div className="mt-5 pt-4 border-t-2 border-slate-300 dark:border-slate-700 print:border-black">
              <div className="grid grid-cols-3 gap-3 text-center text-xs">
                {/* Box 1 */}
                <div className="p-3 rounded-xl border border-slate-300 dark:border-slate-700 print:border-black min-h-[92px] flex flex-col justify-between bg-white dark:bg-slate-900/40">
                  <span className="font-bold text-[11px] text-slate-700 dark:text-slate-200 print:text-black">
                    Le Parent ou Tuteur Légal
                  </span>
                  <span className="text-[9px] text-slate-400 print:text-slate-600 italic">Mention « Lu et approuvé » & signature</span>
                </div>
                ...
                {/* Box 3 */}
                <div className="p-3 rounded-xl border border-slate-300 dark:border-slate-700 print:border-black min-h-[92px] flex flex-col justify-between bg-white dark:bg-slate-900/40 relative">
                  <div>
                    <span className="font-bold text-[11px] text-slate-700 dark:text-slate-200 print:text-black">
                      Le Chef d'Établissement
                    </span>
                    <p className="text-[10px] font-bold text-[#0071E3] dark:text-[#38BDF8] print:text-black mt-0.5">
                      {directorName}
                    </p>
                  </div>
                  
                  {/* Visual stamp circle mockup */}
                  <div className="absolute right-3 bottom-2 w-14 h-14 rounded-full border border-dashed border-blue-500/40 flex items-center justify-center text-[7px] font-black text-blue-600/60 uppercase rotate-[-12deg] pointer-events-none">
                    ADLON<br/>DIRECTION
                  </div>
              ...
              <div className="mt-3 text-center text-[9.5px] text-slate-500 dark:text-slate-400 print:text-black">
                {schoolName} • Document officiel certifié conforme • Fait à {schoolCity} le {new Date().toLocaleDateString('fr-FR')} • Réf : ADL-{student.matricule}-{selectedTerm.replace(/\s+/g, '')}
              </div>
            </div>
          </div>
        </div>
