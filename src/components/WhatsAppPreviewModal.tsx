...
export const WhatsAppPreviewModal: React.FC<WhatsAppPreviewModalProps> = ({
  ...
  const getInitialMessage = (type: 'relance' | 'convocation' | 'felicitations') => {
    switch (type) {
      ...
      case 'felicitations':
        return `Excellente nouvelle pour la famille de ${student.firstName} ${student.lastName} !\n\nNous avons l'honneur de vous féliciter au nom de ${schoolName} pour les remarquables résultats académiques de votre enfant au conseil de classe (Moyenne : ${student.gpa}/20 - Classé(e) ${student.classRank === 1 ? '1er' : `${student.classRank}ème`} sur ${student.totalStudentsInClass || 1} élèves en ${student.classLevel}).\n\nL'établissement salue son engagement ainsi que votre accompagnement exemplaire !\n\nBien cordialement,\nLa Direction Pédagogique - ${schoolName}`;
    }
  };
